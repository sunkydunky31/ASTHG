...
class Mods
{
	...
	inline public static function parseList():ModsList {
		...
		#if MODS_ALLOWED
		try {
			for (mod in CoolUtil.coolTextFile('${modListConfig.get("File")}.${modListConfig.get("Extension")}'))
			{
				//trace('Mod: $mod');
				if(mod.trim().length < 1) continue;

				var dat = mod.split(modListConfig.get("Split"));
				list.all.push(dat[0]);
				if (dat[1] == modListConfig.get("Enabled"))
	...
	private static function updateModList()
	{
		...
		try {
			for (mod in CoolUtil.coolTextFile('${modListConfig.get("File")}.${modListConfig.get("Extension")}'))
			{
				var dat:Array<String> = mod.split(modListConfig.get("Split"));
				...
				{
					added.push(folder);
					list.push([folder, (dat[1] == modListConfig.get("Enabled"))]);
				}
			}
		} catch(e) {
			trace(e);
		...
		for (values in list)
		{
			...
			fileStr += values[0] + modListConfig.get("Split") + (values[1] ? modListConfig.get("Enabled") : modListConfig.get("Disabled"));
		}

		File.saveContent('${modListConfig.get("File")}.${modListConfig.get("Extension")}', fileStr);
		updatedOnState = true;
	...
	public static function getModEnabled(mod:String):Bool {
		...
		if (modList.enabled.contains(mod))
			return true;
